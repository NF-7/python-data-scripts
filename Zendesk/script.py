    ...
    for keyword in keywords:
        for col in df.columns:
            if keyword.lower() in col.lower():
                return col
    raise ValueError(f"None of the keywords {keywords} found in columns.")
...
def process_tickets(file_path):
    ...
    # Find the relevant columns by keyword
    created_col = find_column(df, ['Created at', 'Date created', 'Creation date'])
    solved_col = find_column(df, ['Solved at', 'Date solved', 'Resolution date'])
    on_hold_time_col = find_column(df, ['On hold time in minutes', 'Hold time'])
    first_reply_time_col = find_column(df, ['First reply time in minutes', 'Reply time'])

    # Convert 'Created at' and 'Solved at' to datetime and normalize to 00:00:00
    df[created_col] = pd.to_datetime(df[created_col]).dt.normalize()
    df[solved_col] = pd.to_datetime(df[solved_col]).dt.normalize()

    # Update 'Requester wait time in minutes' for unresolved tickets
    today = pd.Timestamp.now().normalize()
    unsolved_mask = df[solved_col].isna()
    df.loc[unsolved_mask, 'Requester wait time in minutes'] = (today - df[created_col]).dt.total_seconds() / 60

    # Convert 'First reply time in minutes' to hours
    df['First reply time in hours'] = df[first_reply_time_col] / 60.0

    # Generate the full date range for the DataFrame columns
    min_date = df[created_col].min()
    max_date = df[solved_col].max() if df[solved_col].notna().any() else today  # Use today if some tickets are unsolved
    ...
    metrics = [
        'FIRST REPLY TIME (average hours)', 'FIRST REPLY TIME (worst 10PCT hours)',
        'OPEN TIME (average hours)', 'OPEN TIME (average hours without ON HOLD tickets)',
        'OPEN TIME (worst 10PCT hours)', 'OPEN TIME (median hours)',
        'MONTHLY AVG RESOLUTION TIME (hours)', 'MONTHLY AVG RESOLUTION TIME (average hours without ON HOLD tickets)',
        'RESOLUTION TIME (average hours)',
        'RESOLUTION TIME (average hours without ON HOLD tickets)', 'RESOLUTION TIME (worst 10PCT hours)', 'RESOLUTION TIME (quickest 25PCT hours)'
    ]
    ...
    for date in all_dates:
        open_tickets = df[(df[created_col] <= date) & ((df[solved_col] > date) | (df[solved_col].isna()))]
        resolved_tickets = df[df[solved_col] == date]
        created_tickets = df[df[created_col] == date]

        # Calculate resolution time for resolved tickets
        resolved_times = (resolved_tickets[solved_col] - resolved_tickets[created_col]).dt.total_seconds() / 3600  # Convert to hours
        resolved_times_wo_on_hold = resolved_times[resolved_tickets[on_hold_time_col] == 0]
        ...
        open_times = (date - open_tickets[created_col]).dt.total_seconds() / 3600  # Convert to hours
        open_times_wo_on_hold = open_times[open_tickets[on_hold_time_col] == 0]
    ...
    output_df.loc['FIRST REPLY TIME (average hours)', :] = avg_first_reply_time_series
    output_df.loc['FIRST REPLY TIME (worst 10PCT hours)', :] = worst_10_first_reply_time_series
    output_df.loc['OPEN TIME (average hours)', :] = avg_open_time_series
    output_df.loc['OPEN TIME (average hours without ON HOLD tickets)', :] = avg_open_time_wo_on_hold_series
    output_df.loc['OPEN TIME (worst 10PCT hours)', :] = worst_10_open_time_series
    output_df.loc['OPEN TIME (median hours)', :] = median_open_time_series
    # output_df.loc['TOP 25PCT BEST OPEN TIME (hours)', :] = top_25p_best_open_time_series

    # Calculate monthly average resolution time
    df['Month'] = df[solved_col].dt.to_period('M')
    monthly_avg_res_time = df.dropna(subset=[solved_col]).groupby('Month').apply(
        lambda x: (x[solved_col] - x[created_col]).dt.total_seconds().mean() / 3600
    )

    # Calculate monthly average resolution time for tickets without on-hold tickets
    monthly_avg_res_time_wo_on_hold = df[(df[solved_col].notna()) & (df[on_hold_time_col] == 0)].groupby('Month').apply(
        lambda x: (x[solved_col] - x[created_col]).dt.total_seconds().mean() / 3600
    )

    # Add Monthly Average Resolution Time to output_df
    for period, value in monthly_avg_res_time.items():
        next_month = (period + 1).start_time
        output_df.loc['MONTHLY AVG RESOLUTION TIME (hours)', next_month] = value

    for period, value in monthly_avg_res_time_wo_on_hold.items():
        next_month = (period + 1).start_time
        output_df.loc['MONTHLY AVG RESOLUTION TIME (average hours without ON HOLD tickets)', next_month] = value

    output_df.loc['RESOLUTION TIME (average hours)', :] = avg_res_time_series
    output_df.loc['RESOLUTION TIME (average hours without ON HOLD tickets)', :] = avg_res_time_wo_on_hold_series
    # output_df.loc['MEDIAN RESOLUTION TIME (hours)', :] = median_res_time_series
    output_df.loc['RESOLUTION TIME (worst 10PCT hours)', :] = worst_10_res_time_series
    output_df.loc['RESOLUTION TIME (quickest 25PCT hours)', :] = top_25p_res_time_series
